...
for x in range(len(puzzleList)):
    ...
    startTime = time.time()
    AStar(openList, closedList, goalState1, goalState2, startTime, x)

totalTimeEnd = time.time() - averageTimeStart
averageTimeEnd = totalTimeEnd/50

with open(r'analysis/Astar/timeFile', 'w') as f:
...
for x in range(len(puzzleList)):
    solutionFileName = generateFileName(x, 'Astar', 'Solution.txt')
    searchFileName = generateFileName(x, 'Astar', 'Search.txt')
    countFileLines(searchFileName, solutionFileName)
    countCosts(solutionFileName)

writeLineCountToCountFile(2, numberOfPuzzles)
writeToNoSolutionFile(2)
writeToFileTotalCost(2, numberOfPuzzles)
